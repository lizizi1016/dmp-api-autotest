...
@when(u'I found {count:int} Ushard group with Ushard instance, or I skip the test')
...
def step_impl(context):
	...
	serverXml = serverXml.replace("</dble:server>", """
	...
	schemaXml = schemaXml.replace("</dble:schema>", """
	...
	ruleXml = ruleXml.replace("</dble:rule>", """
    <tableRule name="{0}">
        <rule>
            <columns>id</columns>
	...
	api_request_post(context, "ushard_deploy/save_config", {
		"is_sync": True,
		"group_id": context.ushard_group["group_id"],
