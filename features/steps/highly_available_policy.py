...
def step_impl(context, type):
    rto_template_name = "api_{0}_".format(type) + generate_id()
    template_params = {
...
@then(u'the {type:string} template {should_or_not:should_or_not} be removed successfully in {duration:time}')
def step_imp(context, type, should_or_not, duration):
    ...
    def remove_template(context, flag):
        ...
        match = pyjq.first(
            '.[] | select(."name"=="{0}") | select(."sla_type"=="{1}")'.format(context.rto_template_name, type), res)
