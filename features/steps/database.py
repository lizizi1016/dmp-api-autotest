...
def step_imp(context):
    ...
    for server in servers:
        ...
        if i % 2 == 0:
            j = int(i / 2)
            group_id = "{0}-{1}".format(group, j)
            mysql_alias = "{0}-{1}".format(mysql, j)
            role = 'master'
        ...
        if i % 2 == 1:
            ...
            context.batch_group.append(group_id)
        csv_content_one = """
{group_id},{mysql_alias},{server_id},{server_ip},{role},uguard_semi_sync,{port},,5.7.21,mysql-5.7.21-linux-glibc2.12-x86_64.tar.gz,action,universe_op,universe_pass,my.cnf.5.7,/opt/mysql/etc/{port}/my.cnf,/opt/mysql/backup/{port},/opt/mysql/base/5.7.21,/opt/mysql/data/{port},/opt/mysql/log/binlog/{port},/opt/mysql/log/relaylog/{port},/opt/mysql/log/redolog/{port},/opt/mysql/tmp/{port},TRUE,FALSE,TRUE,SLA_RPO_sample,,90,mysql,mysql,{port},{port},,,/opt/mysql/data/{port}/mysqld.sock,应用英文名,应用名,数据库用途,应用节点描述,应用等级,灾备等级,域名改造,FALSE,应用tag,用途tag,英文简称,应用节点描述,用途,高可用方式,域名改造,操作系统,物理虚拟,创建时间,上线时间,备注,0 0 1 ? *,,0 0 23 1/1 * ?,,on,,,,,,,,""".format(
            group_id = group_id, 
